...
@section('content')
    ...
    <div class="header-container">
        ...
        @endif
<form action="{{route('municipios.store')}}" method="POST" class="row col-lg-12" style="border-radius: 1rem;background-color: white;box-shadow: 0 0 2px 1px #c3c3c3; margin-left: 3px; padding: 25px 15px;">
           @csrf
                ...
                <div class="form-group row" style="margin-top: 1rem;">
                    ...
                    <div class="col-sm-8 input-group ">
                        <select name="departamento" id="" class="form-control">
                            <option value="" selected disabled>Escoger un departamento</option>
                            @forelse ($departament as $item)
                        <option value="{{$item->id}}">{{$item->nombre}}</option>
                            @empty
                            <option value="" selected disabled>No hay datos en departamento</option>
                            @endforelse
