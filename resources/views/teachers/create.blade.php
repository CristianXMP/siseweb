...
<form action="{{route('profesores.store')}}" method="POST" class="row col-lg-12" style="border-radius: 1rem;background-color: white;box-shadow: 0 0 2px 1px #c3c3c3; margin-left: 3px; padding: 25px 15px;">
     @csrf
                ...
                <div class="form-group row" style="margin-top: 1rem;">
                    ...
                    <div class="col-sm-8 input-group ">
                        <select name="municipio" id="" class="form-control">
                            <option value="" selected disabled>Escoge municipio</option>

                          @forelse ($city as $item)
                        <option value="{{$item->id}}">{{$item->nombre}} - {{$item->abreviatura}}</option>
                          @empty
                          <option value="" selected disabled>No hay ciudades para relacionarc</option>
                          @endforelse
